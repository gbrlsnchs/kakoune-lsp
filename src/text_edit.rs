...
pub fn apply_text_edits_try_deferred<T: TextEditish<T>>(
    ...
    let buffile = uri_to_file_path(&uri).to_string_lossy().into_owned();
...
pub fn apply_text_edits_to_file<T: TextEditish<T>>(
    ...
    let filename = uri_to_file_path(uri).to_string_lossy().into_owned();

    let path = std::ffi::CString::new(filename.as_str()).unwrap();
    let mut stat;
    if unsafe {
        stat = std::mem::zeroed();
...
pub fn apply_text_edits_to_buffer<T: TextEditish<T>>(
    ...
    let maybe_buffile = uri.map(|uri| uri_to_file_path(&uri).to_string_lossy().into_owned());
...
mod tests {
    ...
    pub fn lsp_text_edits_to_kakoune_issue_848() {
        ...
        let expected = indoc!(
            r#"select 6.11,6.1000000
               execute-keys "c
                 <esc>"
               select 5.11,5.1000000
               execute-keys "c
                 <esc>"
               select 4.11,4.1000000
               execute-keys "c
                 <esc>"
               select 3.11,3.1000000
               execute-keys "c
                 <esc>"
               select 2.12,2.1000000
               execute-keys "c
                 <esc>"
               select 2.11,2.11
               execute-keys "i <esc>""#
